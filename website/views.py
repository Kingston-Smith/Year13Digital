...
def forum():
    if request.method == "POST":
        ...
        if not title:
            ...
            post = Post(title=title, content=content, author=current_user.id, date_created="{:%B %d, %Y %H:%M:%S}".format(
                datetime.now()), last_updated="{:%B %d, %Y %H:%M:%S}".format(datetime.now()))
            db.session.add(post)
            db.session.commit()
...
def update_post(id):
    ...
    if not post:
        ...
        if not form.title.data or not form.content.data:
            flash('You can not make one of the fields empty', category='error')
            return redirect(url_for('views.blog'))
        elif post.title != form.title.data or post.content != form.content.data:
...
def update_comment(id):
    ...
    if not comment:
        ...
        flash("You can't update someone elses comment unless its on your post",
              category="error")
    elif form.validate_on_submit():
        if not form.content.data:
            flash("You can't make it blank", category='error')
            return redirect(url_for('views.blog'))
        elif comment.text != form.content.data:
            comment.text = form.content.data
            comment.last_updated = "{:%B %d, %Y %H:%M:%S}".format(
                datetime.now())
            db.session.commit()
            flash('Comment updated successfully', category='error')
        else:
...
def create_comment(post_id):
    ...
    if not text:
        ...
        if post:
            comment = Comment(text=text, author=current_user.id, post_id=post_id, date_created="{:%B %d, %Y %H:%M:%S}".format(
                datetime.now()), last_updated="{:%B %d, %Y %H:%M:%S}".format(datetime.now()))
            db.session.add(comment)
            db.session.commit()
            flash('Comment added', category='success')
        else:
            flash('The post you are trying to comment on does not exist',
                  category='error')
    return redirect(url_for('views.blog'))
...
def delete_comment(comment_id):
    ...
    if not comment:
        ...
        flash("You must either own the post or the comment to delete a comment",
              category="error")
    else:
        db.session.delete(comment)
        db.session.commit()
...
def like(post_id):
    ...
    if not post:
        ...
        like = Like(author=current_user.id, post_id=post_id,
                    date_created="{:%B %d, %Y %H:%M:%S}".format(datetime.now()))
        db.session.add(like)
        db.session.commit()
